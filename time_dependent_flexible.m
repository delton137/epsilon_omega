...
% input = load('TTM3Fdata/out_TTM3F_128_1_220_501_tot_dip.dat', 'r');
% input = load('TTM3Fdata/128_1_270td.dat', 'r');  
% input = load('C:\Users\Owner\Dropbox\DIELECTRIC\Dielectric_constant\time_dependent_dielectric\TIP4P2005flex_data\spcorr_1_220f.xvg', 'r');
% input = load('C:\Users\Owner\Dropbox\DIELECTRIC\Dielectric_constant\time_dependent_dielectric\diff_dens\dipcorr_1_240f.xvg', 'r');
% input = load('TTM3Fdata/seperate_files_which_have_been_added\out_128_TTM3F_1_300_tot_dip.dat', 'r');
% input = load('C:\Users\Owner\Dropbox\RESEARCH\eps_omega\dipcorr_TIP4P2005_cube.xvg', 'r');
% input = load('C:\Users\Owner\Dropbox\RESEARCH\eps_omega\dipcorr_TIP4P2005_512.xvg', 'r');
...
epsilon_0 = 100;      %58.8;    %eps(0) to input
...
params0 = [1, 12]; % initial trial parameters
params_str0 = [.97,.99,74];


%Fitting parameters to be put in by hand !
start =  .6/.0005;      % timestep to start fitting
endt  =  1.5/.0005;   % timestep to stop fitting
...
numfreqs = 1000; % number of points to calculate eps_omega at (spacedl ogrithmically)
minfreq = 1/(ntimesteps*timestep);
maxfreq = 1/(2*timestep);

loaddata    = 0;       %can be a time saver when debugging
...
running_epsT = eps0_skip*(1:eps0_points)*timestep/( 10^(-15) );
runnint_epsT = running_epsT./(1000*1000*1000000); %fs -> ns
running_eps = zeros(eps0_points,1);

for i = 1:eps0_points 
      nps = eps0_skip*i;      
    
      AvgDipSqT = sum(sum(input(1:nps,:).^2));
      AvgDipT = sum(input(1:nps,:),1);
...
correlation = correlation/AvgDipSq;
end %if (GROMACS == 0)
...
if printdata == 1 
	...
	save -ascii eps_omega_TTM3F.dat matrix2save 	
end
...
if printrunningeps0 == 1 
	matrix2save = [running_epsT', running_eps];
